...
kmeans = KMeans(n_clusters=250).fit(tf_train_features)

correct = 0
for idx in range(0, len(tf_test_features)):
  predict = kmeans.predict([tf_test_features[idx]])[0]
  expected = tf_test_labels[idx]
  if predict == expected:
    correct += 1

print("KMeans accuracy:")
print(correct / len(tf_test_features))